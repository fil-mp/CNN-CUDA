...
% conv_0_tensorflow_file = trun(dlmread('conv1_layer.txt'.*N);

conv_0_tensorflow = reshape(conv_0_tensorflow_file.', [76,76,8]);

conv_1_tensorflow_file = dlmread('conv2_layer.txt');
% conv_1_tensorflow_file = trun(dlmread('conv2_layer.txt').*N);
...
dense_0_weights_tensorflow_file = dlmread('dense_weights.txt');
dense_1_weights_tensorflow_file = dlmread('dense1_weights.txt');
dense_0_bias_tensorflow_file = dlmread('dense_bias.txt');
dense_1_bias_tensorflow_file = dlmread('dense1_bias.txt');
...
% 
% input_image_file = trun(dlmread('input_layer.txt').*N);
% input_image = reshape(input_image_file.', [80,80,3]);

input_image_file = dlmread('input_layer.txt');
input_image = reshape(input_image_file.', [80,80,3]);
...
for i=1:8 %% different filters loop
    for j=1:3 %% different channels loop
        ...
        conv_0(:,:,i) = conv_0(:,:,i) + conv_2d_custom(image_flattened,kernel_0_flattened,80,5);
...
%     conv_0_down(:,:,i) =  floor(conv_0_bias(:,:,i)./N);
    conv_0_relu(:,:,i) = matrix_2d_relu(conv_0_bias(:,:,i));
...
for i=1:8 %% different filters loop
    for j=1:8 %% different channels loop
        ...
        conv_1(:,:,i) = conv_1(:,:,i) + conv_2d_custom(pooling_0_flattened,kernel_1_flattened,19,4);
    end
...
%     conv_1_down(:,:,i) =  floor(conv_1_bias(:,:,i)./N);
    conv_1_relu(:,:,i) = matrix_2d_relu(conv_1_bias(:,:,i));
end
...
final_output = fully_conected_no_q(pool_layer_output_flattened);
